...
public class FeedFragment extends Fragment {
    ...
    private final List<com.app.mycity.data.model.Issue> allIssues = new ArrayList<>();
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        b.etSearchTitle.addTextChangedListener(new TextWatcher() {
            @Override public void beforeTextChanged(CharSequence s, int st, int c, int a) { }
            @Override public void onTextChanged(CharSequence s, int st, int b0, int c) { }
            @Override public void afterTextChanged(Editable s) {
                titleQuery = s.toString().trim().toLowerCase(Locale.ROOT);
                applySearchFilter();
            }
        });
    ...
    private void applySearchFilter() {
        if (b == null) return;
        List<com.app.mycity.data.model.Issue> filtered = new ArrayList<>();
        if (titleQuery.isEmpty()) {
            filtered.addAll(allIssues);
        } else {
            for (com.app.mycity.data.model.Issue issue : allIssues) {
        ...
        b.tvEmpty.setText(allIssues.isEmpty() ? getString(com.app.mycity.R.string.empty_feed) : "По вашему запросу ничего не найдено");
    }
