...
public class UserRepository {
    ...
    public Task<Void> blockTemporary(String uid, String reason, Date blockedUntil, String adminUid) {
        Map<String, Object> data = new HashMap<>();
        data.put("blockedType", UserProfile.BLOCK_TEMPORARY);
        data.put("blockedReason", reason);
        data.put("blockedUntil", blockedUntil);
        data.put("blockedAt", FieldValue.serverTimestamp());
        data.put("blockedByUid", adminUid);
        return doc(uid).set(data, SetOptions.merge());
    }

    public Task<Void> blockPermanent(String uid, String reason, String adminUid) {
        Map<String, Object> data = new HashMap<>();
        data.put("blockedType", UserProfile.BLOCK_PERMANENT);
        data.put("blockedReason", reason);
        data.put("blockedUntil", null);
        data.put("blockedAt", FieldValue.serverTimestamp());
        data.put("blockedByUid", adminUid);
        return doc(uid).set(data, SetOptions.merge());
    }

    public Task<Void> unblock(String uid) {
        Map<String, Object> data = new HashMap<>();
        data.put("blockedType", UserProfile.BLOCK_NONE);
        data.put("blockedReason", null);
        data.put("blockedUntil", null);
        data.put("blockedAt", null);
        data.put("blockedByUid", null);
        return doc(uid).set(data, SetOptions.merge());
    }
