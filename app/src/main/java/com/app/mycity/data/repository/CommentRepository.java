...
public class CommentRepository {
    ...
    public Task<Void> upsert(String issueId, Comment comment, OnDone onDone) {
        boolean isNew = comment.getId() == null || comment.getId().isEmpty();
        DocumentReference ref = isNew ? col(issueId).document() : col(issueId).document(comment.getId());
        if (isNew) comment.setId(ref.getId());
    ...
    public Task<Void> delete(String issueId, String commentId, OnDone onDone) {
        return col(issueId).document(commentId).delete()
