...
- Описание, адрес (без сырых координат).
- Превью карты 200dp (osmdroid, без жестов) + прозрачный `map_click_overlay` — клик открывает `MapFullscreenFragment`.
...
- **2026-04-19 · G19.6** — Follow-up: вводимый текст в `TextInputEditText` в login/register/forgot_password (и в `bottom_sheet_edit_profile`) отображался тёмным: `Material Components` для `TextInputEditText` берёт цвет из `colorOnSurface` темы, а не из `android:textColor` родителя. Фиксы: (1) в `Widget.MyCity.TextInput` добавлен `materialThemeOverlay=@style/ThemeOverlay.MyCity.TextInputEditText`, новый overlay переопределяет `colorOnSurface`, `colorControlActivated`, `android:textColor` на `@color/text_primary`. (2) В `activity_login.xml`, `activity_register.xml`, `activity_forgot_password.xml` все `TextInputLayout` переключены со стиля `Widget.MaterialComponents.TextInputLayout.OutlinedBox` на `@style/Widget.MyCity.TextInput`, чтобы получить overlay. (3) Все хардкоженные `#F1F5F9` заменены на canonical `@color/text_primary`.
- **2026-04-19 · G19.5** — Follow-up: для гостя `btn_guest_exit` возле аватара больше не показывается (он уезжал под слоган и ломал лейаут toolbar'а). Вместо этого для гостя переиспользуется `btn_bell` справа — иконка заменяется на `ic_home`, tint на `accent_blue`, badge скрыт, клик теперь делает `session.clear()` + перезапуск `SplashActivity`. Для залогиненного пользователя колокольчик ведёт себя как раньше (`NotificationsFragment`). `btn_guest_exit` оставлен в layout скрытым на будущее.
- **2026-04-19 · G19.4** — Follow-up: (1) Настоящая причина, почему маркер на мини-карте не появлялся после возврата с fullscreen — поле `pickerMarker` в `CreateIssueFragment` переживает `onDestroyView` и ссылается на уничтоженный `MapView`. При pop back фрагмент пересоздаёт view (новый `mapPicker`), а `updateMarker` по `pickerMarker != null` пытался переставлять старый Marker, который в `overlays` нового mapPicker не был добавлен. Фикс: `onDestroyView` теперь обнуляет `pickerMarker`, чтобы следующий `updateMarker` создал новый маркер и добавил его в новый `mapPicker.getOverlays()`. (2) В `MapFullscreenFragment` добавлена панель сверху с адресом. `fragment_map_fullscreen.xml` — верхние элементы (btn_back + tv_address) переупакованы в горизонтальный `LinearLayout`, `tv_address` висит справа от кнопки «назад», background = `bg_card`, ellipsize=end, 2 строки. `MapFullscreenFragment.resolveAddress(lat, lng)` — reverse geocoding через Android `Geocoder` в фоновом потоке с `addressSeq` для защиты от устаревших ответов. Вызывается при `onViewCreated` и при каждом новом тапе в editable-режиме; сам себя показывает как «Определяем адрес…» → реальный адрес или «Адрес не найден».
- **2026-04-19 · G19.3** — Follow-up: `CreateIssueFragment.updateMarker` — маркер на мини-карте `mapPicker` не всегда отрисовывался при получении координат через `setFragmentResultListener` (мини-карта ещё не успевала пройти layout на момент вызова). Теперь: `animateTo` заменён на `setCenter`, центровка и `invalidate()` обёрнуты в `mapPicker.post(...)`, маркеру выключен `InfoWindow`.
- **2026-04-19 · G19.2** — Follow-up: полноэкранная карта теперь умеет режим выбора точки. (1) `MapFullscreenFragment` — добавлены `ARG_EDITABLE`, `newInstanceEditable(lat, lng, title)`. В editable-режиме к `MapView.getOverlays()` добавляется `MapEventsOverlay` с `singleTapConfirmedHelper`, который переставляет маркер и запоминает `currentLat/currentLng` + выставляет флаг `picked`. В `onDestroyView` — если `editable && picked`, через `getParentFragmentManager().setFragmentResult(RESULT_KEY, bundle{RESULT_LAT, RESULT_LNG})` возвращает выбранную точку родителю. (2) `MainActivity.openMapPicker(lat, lng)` — новая перегрузка, открывает editable-экземпляр. (3) `CreateIssueFragment` — кнопка «Открыть на весь экран» теперь зовёт `openMapPicker`, а не `openMapFullscreen`. Перед открытием вызывается `persistDraft()`, чтобы координаты не потерялись при `replace` фрагмента; `persistDraft()` расширен, теперь `pendingLat/Lng` сами по себе считаются контентом для сохранения. `onViewCreated` регистрирует `setFragmentResultListener` на `MapFullscreenFragment.RESULT_KEY` с `viewLifecycleOwner`, колбэк вызывает `setCoords(lat, lng)`.
- **2026-04-19 · G19.1** — Follow-up: (1) в `fragment_create_issue.xml` текстовая кнопка `btn_pick_location` переименована «Моё местоположение» → «Открыть на весь экран»; её обработчик в `CreateIssueFragment` теперь вызывает новый `openPickerFullscreen()` → `MainActivity.openMapFullscreen(pendingLat|MOGILEV_LAT, pendingLng|MOGILEV_LNG, "")`. FAB `btn_my_location` внутри карты остался как «Моё местоположение» (запрос GPS). (2) `fragment_map_fullscreen.xml` — иконка у `btn_back` заменена `ic_close` → `ic_back` (обычная стрелка «назад», чтобы возвращаться в форму новой заявки).
- **2026-04-19 · G19** — Догоняющий пакет №3: (1) `activity_main.xml` + `MainActivity` — `fab_admin_divider` полностью удалён (позиционирование так и не получалось стабильно прижать к FAB-стеку); все упоминания вычищены из `expandFab/collapseFab`. (2) Тёмные инпуты — теперь хинт и counter читаемы: в `values/themes.xml` и `values-night/themes.xml` у `Theme.MyCity` задан `android:textColorHint=@color/text_secondary` и глобальный `textInputStyle=@style/Widget.MyCity.TextInput`. Новый стиль наследуется от `Widget.MaterialComponents.TextInputLayout.OutlinedBox` и задаёт `boxBackgroundColor`, `boxStrokeColor`, `hintTextColor=text_primary` (активный), `android:textColorHint=text_secondary` (неактивный), `counterTextColor=text_secondary`, `counterOverflowTextColor/errorTextColor=accent_red`, `helperTextTextColor=text_secondary`. В `fragment_admin_users.xml` у `et_search` хинт переведён с невидимого `@color/text_hint` на `@color/text_secondary`. (3) `fragment_map.xml` — убран блок `LinearLayout@id/filters` с чипами `chip_date` / `chip_comments`: на карте сортировка/фильтры не нужны, они остаются только в ленте. (4) `CreateIssueFragment.restoreDraft()` — убран `toast("Черновик восстановлен")`: очевидное поведение, лишний шум. (5) Fullscreen-карта в `IssueDetailFragment` — отказались от ловли клика по `map_click_overlay` (на разных устройствах и с учётом `MapView` не работало стабильно). В `fragment_issue_detail.xml` overlay и `clickable/focusable/foreground` на `map_container` удалены; вместо них в правом-верхнем углу контейнера появилась отдельная `ImageButton@id/btn_map_fullscreen` (40×40dp, `ic_fullscreen`, полупрозрачный чёрный фон `#AA000000`). `IssueDetailFragment.renderIssue()` — `GestureDetector` + `setOnTouchListener` сняты, вместо них простой `btnMapFullscreen.setOnClickListener` → `MainActivity.openMapFullscreen(lat, lng, title)`; импорт `android.view.MotionEvent` удалён. Новый drawable `ic_fullscreen.xml`.
- **2026-04-19 · G18** — Догоняющий пакет №2: (1) `IssueDetailFragment` — клик по карте полностью переписан: на `map_click_overlay` теперь висит `GestureDetector.SimpleOnGestureListener` (`onDown=true`, `onSingleTapUp` → `openMapFullscreen`). Overlay — самый верхний ребёнок в FrameLayout + `clickable/focusable=true`, поэтому перехватывает все касания до `MapView`, любая точка блока карты открывает fullscreen. (2) `CreateIssueFragment.resolveAddress()` — теперь двойной пайплайн: Android `Geocoder` в фоне (быстрый, локальный) + Nominatim (точный); кто ответил первым непустым адресом — тот и выигрывает, второй не перетирает. Добавлен `bestKnownAddress()` который на submit() fallback'ом берёт уже отрендеренный `tvAddress.getText()`, если `pendingAddress` всё ещё пуст. (3) `activity_main.xml` — `fab_admin_divider` переведён с абсолютного `marginEnd` на `constraintStart/End_toStartOf/EndOf=@id/fab_admin_moderate`, центр гарантированно совпадает с центром FAB независимо от `ensureMinTouchTargetSize`; вертикаль — `constraintBottom_toTopOf=@id/fab_admin_moderate` с `marginBottom=4dp`. (4) `MainActivity.setupPager` — `ViewPager2.OnPageChangeCallback` выключает `setUserInputEnabled` на странице карты (position=1): свайп влево не ломает панорамирование карты; возврат в feed — через FAB «Home» или слоган. (5) **Архив полностью удалён**: удалён `ArchiveFragment.java` и пакет `ui/archive/`, `fab_archive` из layout, ветка в `MainActivity` (импорт, click, `animateMiniFab`, `applyFabTint`, `refreshActiveFab`, `collapseFab`). Оставшиеся FAB подтянуты: moderate/users/stats теперь на 160/205/250dp (было 210/255/300), между ними и add образовался единый gap. (6) `AdminModerationFragment` — добавлено поле поиска (`TextInputLayout` + `TextInputEditText` `et_search`, `imeOptions=actionSearch`). Входящий список из Firestore кэшируется в `allIssues`, отрисовывается через `applyFilter()` который фильтрует по `title.contains(query)` case-insensitive; `tv_empty` синхронно переключается.
- **2026-04-19 · G17** — Догоняющий пакет: (1) `IssueDetailFragment` — клик по карте теперь вешается сразу на три слоя (`mapContainer`, `mapClickOverlay` и `setOnTouchListener` на сам `mapView`, ловит `ACTION_UP`); добрались наконец до `MapFullscreenFragment`. (2) `MainActivity.setupToolbar` — `tv_title` (слоган «Мой город — моя ответственность») сделан кликабельным, клик → `popHostToRoot()` + `viewPager.setCurrentItem(0)`. (3) `GeoUtils.displayAddress(String)` — новая хелпер-функция: распознаёт строки в формате `lat, lng` (regex) и заменяет их на «Без адреса». Применено в `IssueCardAdapter`, `IssueDetailFragment`, `MapFragment.showPopup` — старые заявки с координатами вместо адреса теперь больше не позорятся. (4) `NominatimClient` — добавлен `OkHttpClient` с таймаутами 15s (connect/read/write). `NominatimApi` — добавлены `zoom=18&addressdetails=1`. `NominatimResponse.shortAddress()` — расширен fallback: road → pedestrian/footway/path/residential; при отсутствии улицы возвращает `neighbourhood/quarter/suburb/city_district` + `city/town/village/hamlet`. `CreateIssueFragment.setCoords` — добавлен `Log.d/Log.e` в тег `Nominatim` для диагностики. (5) `NotificationRepository` — `send()` теперь собирает сообщение как «{admin} закрыл заявку «{issueTitle}»» с fallback'ами; добавлены `delete(id)` и `deleteAll(userId)` (batch). (6) `AdminResolveBottomSheet` — fallback имени админа: Firestore displayName → `FirebaseUser.getDisplayName()` → «Администратор» (больше не светит email/UID). (7) `item_notification.xml` — добавлена кнопка `btn_notif_delete` (32dp `ic_delete` borderless). `NotificationAdapter` получил `OnDeleteListener`. `fragment_notifications.xml` — рядом с «Прочитать все» добавлена красная `btn_delete_all` («Очистить»). `NotificationsFragment` — per-item delete с подтверждением и bulk delete с подтверждением через `MaterialAlertDialog`.
- **2026-04-19 · G16** — Пакет фиксов: (1) `IssueDetailFragment` карта — добавлен `map_click_overlay` поверх `MapView`, сам `MapView` переведён в `setMultiTouchControls(false)` + `setClickable(false)`; клик по карте теперь надёжно открывает `MapFullscreenFragment`. (2) `activity_main.xml` — `fab_admin_divider` переехал на `marginBottom=205dp` (раньше был 192dp и полностью перекрывался `fab_archive`), размер увеличен до 32×2dp; в `expandFab()` добавлен `animate().cancel()` перед сбросом альфы, чтобы избежать залипших анимаций. (3) `CreateIssueFragment` — адрес вместо сырых координат: заглушка «Определяем адрес…» пока Nominatim отвечает, «Адрес не найден» при ошибке/пустом ответе; в `Issue.address` сохраняется либо реальный адрес, либо «Адрес не найден». (4) Тост «используются координаты центра города» теперь показывается один раз за запуск (`static boolean locationToastShown`). (5) `fragment_profile.xml` и `fragment_user_profile_view.xml` — FrameLayout аватара увеличен до 110×110dp с `clipChildren="false"` (у родителя тоже), корона без отрицательного маргина (центр-верх), бейдж с числом заявок 26×26dp с отступами — теперь точно не клипится. (6) `CircleImageView iv_avatar` в toolbar — убран `civ_border_color/width` (лишняя серая рамка). (7) `NotificationRepository` — добавлены `Log.d/Log.e` для диагностики проблем с уведомлениями.
